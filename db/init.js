...
// Uses libSQL purely as a LOCAL file database (no Turso/cloud sync) — see note below.
...
// ── Turso connectivity is HARD-DISABLED at the code level ──
// A previously-connected Turso database had pre-existing, incompatible data from
// an unrelated prior system (UUID primary keys, mismatched column names) that kept
// causing corruption-related errors no amount of schema-repair could fully resolve.
// Rather than rely on Render's Environment tab having TURSO_DATABASE_URL/
// TURSO_AUTH_TOKEN removed (easy to miss, or accidentally re-added later), this is
// disabled directly in code — even if those env vars are still set in Render, they
// are IGNORED, and the app always runs on local file storage only.
//
// IMPORTANT TRADE-OFF: Render's free tier has no persistent disk, so local file
// storage means all data is wiped on every redeploy and likely every restart after
// ~15 minutes of inactivity. This was a deliberate, explicit choice to prioritize
// stability over persistence after Turso connectivity caused repeated data
// corruption issues. To safely re-enable Turso in the future: set up a BRAND NEW,
// empty Turso database (not the old one, which has incompatible legacy data) and
// flip TURSO_DISABLED below to false.
const TURSO_DISABLED = true;
const TURSO_URL = TURSO_DISABLED ? null : (process.env.TURSO_DATABASE_URL || null);
const TURSO_TOKEN = TURSO_DISABLED ? null : (process.env.TURSO_AUTH_TOKEN || null);
...
function parseExpectedColumns(schemaSql) {
  ...
  while ((match = tableRegex.exec(schemaSql))) {
    ...
    for (const rawLine of body.split('\n')) {
      ...
      // Detect this column itself being the primary key (e.g. "id INTEGER PRIMARY
      // KEY AUTOINCREMENT" or "key TEXT PRIMARY KEY") — these always already exist
      // on any real table (you can't have a table without its own PK), so trying to
      // ALTER TABLE ADD COLUMN them is always wrong, regardless of what they're named.
      ...
      columns.push({ name: colName, type: normalizeType(colType), defaultClause, isPrimaryKey });
    }
...
function reconcileSchema(wrapper) {
  ...
  for (const [tableName, expectedCols] of Object.entries(expected)) {
    let actualCols;
    try {
      actualCols = wrapper.prepare(`PRAGMA table_info(${tableName})`).all().map(c => c.name.toLowerCase());
    } catch (e) {
    ...
    for (const col of expectedCols) {
      if (col.isPrimaryKey) continue; // every real table already has its own primary key column, by definition
      // Case-insensitive comparison — Turso/SQLite column names aren't case-sensitive
      // for matching purposes, but a naive `.includes()` with mismatched casing (e.g.
      // an existing "KEY" vs expected "key") would otherwise try to re-add it and fail
      // with "duplicate column name".
      if (actualCols.includes(col.name.toLowerCase())) continue;

      // SQLite/Turso's ALTER TABLE ADD COLUMN rejects non-constant defaults like
      // DEFAULT (datetime('now')) — "Cannot add a column with non-constant default".
      // Constant defaults (numbers, quoted strings) work fine inline. For the
      // non-constant case, add the column with no default, then backfill existing
      // rows with an UPDATE so old rows still get a sensible value instead of NULL.
      const isConstantDefault = col.defaultClause && /^('[^']*'|\d+)$/.test(col.defaultClause);
      const inlineDefaultSql = isConstantDefault ? ` DEFAULT ${col.defaultClause}` : '';
      const alterSql = `ALTER TABLE ${tableName} ADD COLUMN ${col.name} ${col.type}${inlineDefaultSql}`;
      ...
      try {
        ...
        if (col.defaultClause && !isConstantDefault) {
          // Non-constant default (e.g. datetime('now')) — backfill separately.
          try {
            wrapper.exec(`UPDATE ${tableName} SET ${col.name} = ${col.defaultClause} WHERE ${col.name} IS NULL`);
          } catch (backfillErr) {
            console.error(`⚠️ Backfill failed for ${tableName}.${col.name} (column was still added successfully):`, backfillErr.message);
          }
        }
      } catch (e) {
...
// ── Factory Reset ──
// Drops every table this app knows about and recreates the schema from scratch,
// clean. This is the real fix for a database that has fundamentally incompatible
// pre-existing data (e.g. a `users` table from a different prior system, with a
// UUID primary key and a `full_name` column instead of `name` — reconciliation
// can only ADD missing columns, it can't fix a wrong primary key type or migrate
// data living under the wrong column name). Admin-only, requires explicit
// confirmation text from the UI to reduce the chance of an accidental click.
function factoryReset() {
  if (!dbInstance) throw new Error('Database not ready yet');
  const expected = parseExpectedColumns(SCHEMA);
  const tableNames = Object.keys(expected);

  // DROP first (children before parents doesn't actually matter here since none of
  // these use SQLite foreign key constraints with cascade), then recreate schema,
  // then reseed the default admin user — same as a brand new install.
  for (const tableName of tableNames) {
    try { dbInstance.exec(`DROP TABLE IF EXISTS ${tableName}`); } catch (e) {
      console.error(`Factory reset: failed to drop ${tableName}:`, e.message);
    }
  }
  dbInstance.exec(SCHEMA);

  const hash = bcrypt.hashSync('admin123', 10);
  dbInstance.prepare('INSERT INTO users (name, username, password_hash, role) VALUES (?, ?, ?, ?)')
    .run('Admin', 'admin', hash, 'admin');
  dbInstance.scheduleSync();

  console.log('✓ Factory reset complete — database rebuilt clean, default admin user recreated.');
  return { tablesReset: tableNames.length };
}
...
const dbProxy = new Proxy({}, {
  get(_target, prop) {
    ...
    if (prop === 'factoryReset') return factoryReset;
