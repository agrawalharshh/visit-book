...
async function saveVisitClient() {
  ...
  const payload = {
    ...
    lead_status: $('vc_lead_status').value, status: $('vc_status').value, remarks: $('vc_remarks').value.trim(),
  };
...
async function markConverted(visitClientId) {
  const c = STATE.visitClients.find(x => x.id === visitClientId);
  if (!c) return;
  if (!confirm(`Move "${c.name}" to Active Clients? This removes them from Client Visits and carries their visit history over.`)) return;
  try {
    await api('POST', `/visit-logs/convert/${visitClientId}`, { monthly_value: 0 });
    await loadVisitClients();
    await loadActiveClients();
    await loadFollowups();
    toast(`⭐ ${c.name} moved to Active Clients`);
    showPage('active');
...
function downloadImportTemplate(table) {
  const templates = {
    visit_clients: { file: 'Visit_Clients_Import_Template.csv', cols: ['name', 'area', 'phone', 'address', 'last_visit', 'follow_up_date', 'lead_status', 'status', 'remarks'], sample: ['Ramesh Patel', 'Varachha', '9876543210', 'Ring Road', '2026-06-21', '2026-06-28', 'Hot', 'Prospect', 'Interested in silk sarees'] },
    active_clients: { file: 'Active_Clients_Import_Template.csv', cols: ['name', 'area', 'phone', 'business', 'address', 'monthly_value', 'status', 'remarks'], sample: ['Suresh Textiles', 'Katargam', '9123456780', 'Wholesale', 'Main Market', '15000', 'Active', 'Regular bulk buyer'] },
    swa_data: { file: 'Swa_Data_Import_Template.csv', cols: ['sr', 'company', 'client', 'phone', 'address', 'remarks'], sample: ['1', 'ABC Textiles', 'Mr Patel', '9876500000', 'Ring Road', 'Urgent'] },
  };
...
async function saveActiveClient() {
  ...
  const payload = {
    ...
    monthly_value: parseFloat($('ac_monthly').value) || 0,
...
function addOrderItemRow() {
  ...
  row.style.cssText = 'grid-template-columns:2fr 1fr 1fr 1fr auto;gap:8px;margin-bottom:8px;align-items:end';
  row.id = id;
  row.innerHTML = `
