...
router.put('/clients/:id', async (req, res) => {
  const c=req.body;
  await run("UPDATE clients SET company=?,client=?,phone=?,address=?,notes=?,grade=?,assigned_to=?,orders=?,updated_at=datetime('now') WHERE id=?",
...
router.post('/clients/:id/orders', async (req, res) => {
  ...
  orders.push(order);
  await run("UPDATE clients SET orders=?,updated_at=datetime('now') WHERE id=?",[JSON.stringify(orders),req.params.id]);
...
router.put('/meetings/:id', async (req, res) => {
  const m=req.body;
  await run("UPDATE meetings SET date=?,time=?,company=?,client=?,phone=?,location=?,agenda=?,status=?,updated_at=datetime('now') WHERE id=?",
...
router.put('/swa/:id', async (req, res) => {
  const s=req.body;
  await run("UPDATE swa_data SET company=?,client=?,phone=?,address=?,remarks=?,status=?,wa_sent=?,selected=?,selected_date=?,updated_at=datetime('now') WHERE id=?",
...
router.put('/config', async (req, res) => {
  for (const [k,v] of Object.entries(req.body)) {
    ...
    if (exists) await run("UPDATE config SET value=?,updated_at=datetime('now') WHERE key=?",[JSON.stringify(v),k]);
