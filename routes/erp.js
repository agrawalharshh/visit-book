...
router.post('/erp/:table', async (req, res) => {
  try {
    ...
    broadcastChange(senderId, req.params.table);
    res.json({ ok: true });
...
function broadcastChange(senderClientId, changedTable) {
  const msg = `event: data-changed\ndata: ${JSON.stringify({ table: changedTable, by: senderClientId, ts: Date.now() })}\n\n`;
  let dead = [];
  sseClients.forEach((res, id) => {
    if (id === senderClientId) return; // don't send back to the sender
    try { res.write(msg); }
