...
// Sorted by AREA first, then date — this matches how sales executives actually
// work: a fixed area each day, so the list should already be grouped that way
// rather than needing manual re-sorting every morning.
router.get('/', (req, res) => {
  const rows = db.prepare(`
    SELECT * FROM visit_clients
    WHERE follow_up_date IS NOT NULL AND follow_up_date != ''
      AND date(follow_up_date) <= date('now')
    ORDER BY area COLLATE NOCASE ASC, date(follow_up_date) ASC
  `).all();
  res.json(rows);
});
