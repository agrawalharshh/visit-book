...
// body: { active_client_id, bill_no, order_date, notes, items: [{product, qty, unit, rate}] }
router.post('/', (req, res) => {
  ...
  const tx = db.transaction(() => {
    ...
    const itemStmt = db.prepare('INSERT INTO order_items (order_id, product, qty, unit, rate, amount) VALUES (?, ?, ?, ?, ?, ?)');
    for (const item of lineItems) {
      const qty = parseFloat(item.qty) || 0;
      const rate = parseFloat(item.rate) || 0;
      const unit = item.unit === 'tons' ? 'tons' : 'kg';
      itemStmt.run(orderId, item.product, qty, unit, rate, qty * rate);
    }
    return orderId;
  });
