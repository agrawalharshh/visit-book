...
// Router-level mount already requires 'view_swa' (see server.js) — this adds the
// stricter 'edit_swa' check needed for anything that creates, moves, or deletes data.
function requireEditSwa(req, res, next) {
  if (!can(req.user.role, 'edit_swa')) return res.status(403).json({ error: `Your role (${req.user.role}) cannot edit Swa data.` });
  next();
}
...
router.post('/data', requireEditSwa, (req, res) => {
...
router.put('/data/:id', requireEditSwa, (req, res) => {
  const { sr, company, client, phone, address, remarks } = req.body;
...
router.delete('/data/:id', requireEditSwa, (req, res) => {
...
router.post('/data/bulk-import', requireEditSwa, (req, res) => {
...
// ── Move ONLY (no WhatsApp send at all) — just shortlist rows into Swa Selected ──
router.post('/move-only', requireEditSwa, (req, res) => {
  const { ids } = req.body;
  ...
  const tx = db.transaction((rows) => {
    for (const r of rows) {
      insertSel.run(r.sr, r.company, r.client, r.phone, r.address, r.remarks, batchId);
      deleteData.run(r.id);
    }
  });
  tx(rows);

  res.json({ success: true, moved: rows.length, batchId });
});
...
router.post('/move-and-send', requireEditSwa, async (req, res) => {
...
router.post('/move-and-send-individual', requireEditSwa, async (req, res) => {
...
router.post('/selected/ai-sort-by-area', requireEditSwa, async (req, res) => {
