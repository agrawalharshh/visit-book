...
router.post('/convert/:visitClientId', (req, res) => {
  ...
  const { monthly_value, converted_by, grade } = req.body || {};

  const tx = db.transaction(() => {
    const info = db.prepare(`INSERT INTO active_clients (name, company, area, address, landmark, location, phone, business, monthly_value, status, grade, converted_by, remarks, converted_from_visit_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', ?, ?, ?, ?)`)
      .run(vc.name, vc.company, vc.area, vc.address, vc.landmark, vc.location, vc.phone, vc.business, monthly_value || 0, grade || null, converted_by || null, vc.remarks, vc.id);
