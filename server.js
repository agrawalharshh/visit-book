...
async function start() {
  ...
  app.use('/api/executives', requireAuth, require('./routes/executives'));
  app.use('/api/backup', requireAuth, require('./routes/backup'));
  app.use('/api/targets', requireAuth, require('./routes/targets'));
  app.use('/api/wa-defaults', requireAuth, require('./routes/waDefaults'));
  ...
  // a chance to run the graceful shutdown handler below. (Belt-and-suspenders —
  // every individual write already saves immediately, see db/sqljs-wrapper.js.)
  setInterval(() => {
