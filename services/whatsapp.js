...
async function syncTemplates() {
  ...
  if (!token) {
    throw new Error('No Access Token saved. Go to Settings → WhatsApp API and paste your permanent token.');
  }
  if (!wabaId) {
    throw new Error('No WhatsApp Business Account ID saved. Go to Settings → WhatsApp API and paste it (found on Meta\'s WhatsApp → API Setup page).');
  }
  // Defensive trim + format check even though settings save now trims on the way
  // in — this catches any value saved before that fix, or pasted from elsewhere.
  const cleanWabaId = wabaId.trim();
  const cleanToken = token.trim();
  if (!/^\d+$/.test(cleanWabaId)) {
    throw new Error(`WhatsApp Business Account ID looks wrong: "${cleanWabaId}". It should be all digits (e.g. 119283746102938), with no spaces or letters. Re-copy it from Meta's WhatsApp → API Setup page and save again.`);
  }

  const url = `https://graph.facebook.com/${GRAPH_VERSION}/${cleanWabaId}/message_templates?limit=100`;
  let res, data;
  try {
    res = await fetch(url, { headers: { Authorization: `Bearer ${cleanToken}` } });
  } catch (networkErr) {
    throw new Error(`Could not reach Meta's servers: ${networkErr.message}. Check your Render service has outbound internet access.`);
  }
  ...
  try {
    ...
    // Meta (or a proxy in between) returned something that isn't JSON at all —
    // surface the raw text and HTTP status so this is diagnosable instead of
    // silently failing with no clue.
    throw new Error(`Meta returned an unexpected response (HTTP ${res.status}): ${rawText.slice(0, 200) || '(empty body)'}`);
  }

  if (data.error) {
    const code = data.error.code ? ` (code ${data.error.code})` : '';
    throw new Error(`Meta API error${code}: ${data.error.message || 'Unknown error'}${data.error.error_subcode ? ` [subcode ${data.error.error_subcode}]` : ''}`);
  }
  if (!Array.isArray(data.data)) {
    throw new Error(`Meta returned no template list — double check your WhatsApp Business Account ID is correct and this app has been granted access to it.`);
  }
